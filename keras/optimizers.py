...
class Optimizer(object):
    ...
    def get_updates(self, params, constraints, lr_multipliers, loss):
...
class SGD(Optimizer):
    ...
    def get_updates(self, params, constraints, lr_multipliers, loss):
        ...
        for p, g, c, mul in zip(params, grads, constraints, lr_multipliers):
            m = K.variable(np.zeros(K.get_value(p).shape))  # momentum
            v = self.momentum * m - (lr * mul) * g  # velocity
            self.updates.append((m, v))

            if self.nesterov:
                new_p = p + self.momentum * v - (lr * mul) * g
            else:
                new_p = p + v
...
class RMSprop(Optimizer):
    ...
    def get_updates(self, params, constraints, lr_multipliers, loss):
        ...
        for p, g, a, c, m in zip(params, grads, accumulators, constraints, lr_multipliers):
            # update accumulator
            new_a = self.rho * a + (1 - self.rho) * K.square(g)
            self.updates.append((a, new_a))

            new_p = p - (self.lr * m) * g / K.sqrt(new_a + self.epsilon)
            self.updates.append((p, c(new_p)))  # apply constraints
        return self.updates
...
class Adadelta(Optimizer):
    ...
    def get_updates(self, params, constraints, lr_multipliers, loss):
        ...
        for p, g, a, d_a, c, m in zip(params, grads, accumulators,
                                   delta_accumulators, constraints, lr_multipliers):
            # update accumulator
            new_a = self.rho * a + (1 - self.rho) * K.square(g)
            self.updates.append((a, new_a))
...
class Adam(Optimizer):
    ...
    def get_updates(self, params, constraints, lr_multipliers, loss):
        ...
        for p, g, c, mul in zip(params, grads, constraints, lr_multipliers):
            ...
            p_t = p - (lr_t * mul) * m_t / (K.sqrt(v_t) + self.epsilon)
...
class Adamax(Optimizer):
    ...
    def get_updates(self, params, constraints, lr_multipliers, loss):
        ...
        for p, g, c, mul in zip(params, grads, constraints, lr_multipliers):
            ...
            p_t = p - (lr_t * mul) * m_t / (u_t + self.epsilon)
